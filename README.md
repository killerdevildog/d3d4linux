...
This is a fork of [d3d4linux](https://github.com/samhocevar/d3d4linux) maintained
for integration with the [bgfx](https://github.com/bkaradzic/bgfx) rendering library.
...
- **Wine 11+ compatibility**: Updated default Wine path to `/usr/bin/wine` with fallback to `wine64` for older versions
- **D3DReflect struct alignment fix**: Added `D3D_MIN_PRECISION` enum and `MinPrecision` field to `D3D11_SIGNATURE_PARAMETER_DESC` to fix struct size mismatch (32→40 bytes) with `d3dcompiler_47.dll`
